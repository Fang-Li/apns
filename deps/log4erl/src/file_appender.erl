...
init({Dir, Fname, {Type, Max}, Rot, Suf, Level, Pattern} = _Conf) ->
	...
	{Y,M,D} = date(),
	DataStr = integer_to_list(Y)++integer_to_list(M)++integer_to_list(D),
    File = Dir ++ "/" ++ Fname ++"_"++DataStr++ "." ++ Suf,
...
rotate(#file_appender{fd = Fd, dir=Dir,  file_name=Fn, counter=Cntr, rotation=Rot, suffix=Suf, log_type=Ltype, level=Level, format=Format} = _S) ->
	...
	{Y,M,D} = date(),
	DataStr = integer_to_list(Y)++integer_to_list(M)++integer_to_list(D),
%%     rotate_file(Dir ++ "/" ++ Fn ++"_"++ DataStr, Rot - 1, Suf),
    Src = Dir ++ "/" ++ Fn ++"_"++ DataStr ++ "." ++ Suf,
    {ok ,Fd2} = file:open(Src, ?FILE_OPTIONS),
