...
-spec connect(
  atom(), fun((binary(), erl4apns:status()) -> stop | _), fun((string()) -> _)) ->
    {ok, pid()} | {error, {already_started, pid()}} | {error, Reason::term()}.
...
-spec connect(fun((binary(), erl4apns:status()) -> stop | _), fun((string()) -> _))->
	{ok, pid()} | {error, {already_started, pid()}} | {error, Reason::term()}.
connect(ErrorFun, FeedbackFun)->
	connect((default_connection())#apns_connection{error_fun = ErrorFun,feedback_fun = FeedbackFun}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
get_env(K, Def) ->
  case application:get_env(erl4apns, K) of
    {ok, V} -> V;
    _ -> Def
  end.
