...
%% @hidden
-spec start_connection(erl4apns:connection()) -> {ok, pid()} | {error, term()}.
start_connection(Connection) ->
  supervisor:start_child(?MODULE, [Connection]).

%% @hidden
-spec start_connection(atom(), erl4apns:connection()) ->
  {ok, pid()} | {error, term()}.
...
-spec init(_) ->
  {ok,
   {{simple_one_for_one, 20, 200},
    [{connection, {apns_connection, start_link, []},
      permanent, 2000, worker, [apns_connection]}]}}.
init(_) ->
  {ok,
   {{simple_one_for_one, 20, 20},
    [{connection, {apns_connection, start_link, []},
      permanent, 2000, worker, [apns_connection]}]}}.
