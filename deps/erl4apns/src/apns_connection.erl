...
-record(state, {out_socket        :: tuple(),
                in_socket         :: tuple(),
                connection        :: erl4apns:connection(),
                in_buffer = <<>>  :: binary(),
                out_buffer = <<>> :: binary(),
                queue             :: pid(),
                out_expires       :: integer()}).
...
%% @hidden
-spec init(erl4apns:connection()) -> {ok, state()} | {stop, term()}.
init(Connection) ->
  try
   log4erl:info("connect info~p", [Connection]),
    ...
    case open_out(Connection) of
      {ok, OutSocket} -> 
          ...
          {ok, InSocket} ->
			%%  apns_pool:checkin(apns_pool, self()),
...
handle_info( {ssl, SslSocket, Data}
  ...
  case <<CurrentBuffer/binary, Data/binary>> of
    <<TimeT:4/big-unsigned-integer-unit:8,
      ...
      Token:Length/binary,
      Rest/binary>> ->
      try Feedback({erl4apns:timestamp(TimeT), bin_to_hexstr(Token)})
      catch
        _:Error ->
          log4erl:error(
...
handle_info(reconnect, State = #state{connection = Connection}) ->
  log4erl:info("Reconnecting the Feedback server...~n"),
  case open_feedback(Connection) of
...
handle_info({ssl_closed, SslSocket}, State = #state{out_socket = SslSocket}) ->
  log4erl:info("APNS disconnected~n"),
  apns_pool:checkin(apns_pool, self()),
...
terminate(_Reason, _State) -> 
	apns_pool:free_conn(apns_pool, self()),
	ok.
...
send_payload(Socket, MsgId, Expiry, BinToken, Payload, Priority) ->
    ...
    Packet = [<<2:8,
                FrameLength:32/big,
                Frame/binary>>],
    log4erl:info("Sending msg ~p (expires on ~p)~n",
                         [MsgId, Expiry]),
