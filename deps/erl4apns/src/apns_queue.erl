...
-record(state, {
    queue = queue:new() :: {list(_Item), list(_Item)},
...
-spec in(QID :: pid(), Msg :: erl4apns:msg()) -> ok.
in(QID, Msg) ->
    gen_server:cast(QID, {in, Msg}).

-spec fail(QID :: pid(), ID :: binary()) -> [erl4apns:msg()].
fail(QID, ID) ->
...
%% @hidden
-spec handle_call(X::term(), reference(), state()) -> {reply, {Failed::erl4apns:msg(), RestToRetry::[erl4apns:msg()]}, state()}.
...
-spec recover_fail(ID::binary(), Queue::{list(_Item), list(_Item)}) -> {erl4apns:msg(), [erl4apns:msg()]}.
%@hidden
recover_fail(ID, Queue) ->
    Now = erl4apns:expiry(0),
